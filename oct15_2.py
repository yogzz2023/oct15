...
def calculate_hypothesis_probability(hypothesis, tracks, reports, kalman_filter):
    probability = 1.0
...
def generate_hypotheses(cluster_tracks, cluster_reports):
    track_indices = range(len(cluster_tracks))
    report_indices = range(len(cluster_reports))
    all_possible_assignments = list(itertools.product(report_indices, repeat=len(track_indices)))

    hypotheses = []
    for assignment in all_possible_assignments:
        hypothesis = [(track_idx, assignment[track_idx]) for track_idx in track_indices]
        hypotheses.append(hypothesis)

    return hypotheses

def perform_jpda_with_hypotheses(tracks, reports, kalman_filter):
    ...
    for cluster_tracks, cluster_reports in clusters:
        ...
        for hypothesis in hypotheses:
            probability = calculate_hypothesis_probability(hypothesis, cluster_tracks, cluster_reports, kalman_filter)
            hypothesis_probabilities.append((hypothesis, probability))

        # Select the hypothesis with the highest probability
        best_hypothesis, best_probability = max(hypothesis_probabilities, key=lambda x: x[1])

        # Use the best hypothesis to select the best report for each track
        for track_idx, report_idx in best_hypothesis:
            best_report = cluster_reports[report_idx]
            best_reports.append((track_idx, best_report))
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) == 1:  # Single measurement
            ...
            if association_method == 'JPDA':
                best_reports = perform_jpda_with_hypotheses([track['measurements'][-1][0][:3] for track in tracks], reports, kalman_filter)
            ...
            for track_id, best_report in best_reports:
                ...
                log_data = {
                    ...
                    'Probability of Hypothesis': str(hypothesis_probabilities),
