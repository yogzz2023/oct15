...
def generate_hypotheses(cluster_tracks, cluster_reports):
    ...
    for perm in permutations(range(num_reports), num_tracks):
        hypothesis = [(track_idx, perm[track_idx]) for track_idx in range(num_tracks)]
        hypotheses.append(hypothesis)

    return hypotheses


def calculate_hypothesis_probabilities(hypotheses, kalman_filter):
    probabilities = []

    for hypothesis in hypotheses:
        probability = 1.0
        for track_idx, report_idx in hypothesis:
            track = kalman_filter.Sf[:3, 0]  # Assuming track is the state vector
            report = kalman_filter.Z  # Assuming report is the measurement vector
            cov_inv = np.linalg.inv(kalman_filter.Pp[:3, :3])
            residual = report - track
...
def perform_jpda(tracks, reports, kalman_filter):
    ...
    for cluster_tracks, cluster_reports in clusters:
        ...
        hypothesis_probabilities = calculate_hypothesis_probabilities(hypotheses, kalman_filter)

        # Select the best hypothesis based on the highest probability
        best_hypothesis_index = np.argmax(hypothesis_probabilities)
        best_hypothesis = hypotheses[best_hypothesis_index]

        # Extract the best reports from the best hypothesis
        for track_idx, report_idx in best_hypothesis:
            best_reports.append((track_idx, cluster_reports[report_idx]))
